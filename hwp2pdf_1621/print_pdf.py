...
def print_pdf(hwp_list):
    ...
    for file in hwp_list:
        ...
        process1 = subprocess.Popen(["python", "ezpdf.py",
                                     ...
                                     str(width), str(height), sys.executable])
    ...
    subprocess.run([sys.executable, "ezpdf_setprop.py", *DEFAULT, sys.executable])
    process3.wait()


if __name__ == '__main__':
    hwp_list = glob(r"C:\Users\user\AppData\Local\Programs\Python\Python312\lab\hwp2pdf_1621\*.hwp")
    print_pdf(hwp_list)
    hwp.quit()
